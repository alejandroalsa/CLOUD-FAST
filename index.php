<?php
  ...
  ini_set('display_errors', 1); 
    ...
    $records_files = $con->query("SELECT * FROM files WHERE user_id = {$_SESSION['user']['id']}");
    $user_files = $records_files->fetchAll(PDO::FETCH_ASSOC);
    foreach ($user_files as &$user_file) {
      $user_file['table_name'] = 'files';
    }
    
    $records_documents = $con->query("SELECT * FROM documents WHERE user_id = {$_SESSION['user']['id']}");
    $user_documents = $records_documents->fetchAll(PDO::FETCH_ASSOC);
    foreach ($user_documents as &$user_document) {
      $user_document['table_name'] = 'documents';
    }
    ...
    $all_user_records = array_merge($user_files, $user_documents, $user_multimedia);
...
<html lang="es">
  ...
  <body>
    ...
    <header class="navbar navbar-dark sticky-top bg-dark flex-md-nowrap p-0 shadow">
      <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3 fs-6" href="index.php">CLOUD~FAST | TODO</a>
        ...
        <nav id="sidebarMenu" class="col-md-3 col-lg-2 d-md-block bg-light sidebar collapse">
          <div class="position-sticky pt-3 sidebar-sticky">
            <ul class="nav flex-column">
              <li class="nav-item">
                <a class="nav-link active" aria-current="page" href="index.php" style="font-size: 16px;">
                  <i class="bi bi-archive-fill"></i>
                  Todo
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/archive.php" style="font-size: 16px;">
                  <i class="bi bi-file-earmark-zip-fill fa-x5"></i>
                  Archivos
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/text.php" style="font-size: 16px;">
                  <i class="bi bi-filetype-txt"></i>
                  Textos
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/multimedia.php" style="font-size: 16px;">
                  <i class="bi bi-file-image"></i>
                  Multimedia
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/documents.php" style="font-size: 16px;">
            ...
            <ul class="nav flex-column mb-2">
              <li class="nav-item">
                <a class="nav-link" href="sections/account.php" style="font-size: 16px;">
            ...
            <ul class="nav flex-column mb-2">
              <li class="nav-item">
                <a class="nav-link" href="sections/privacy-policy.php" style="font-size: 16px;">
                  <i class="bi bi-journal-bookmark-fill"></i>
                  Política de Privacidad
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/version.php" style="font-size: 16px;">
                  <i class="bi bi-git"></i>
                  Versión 1.0
                </a>
              </li>
              <li class="nav-item">
                <a class="nav-link" href="sections/status.php" style="font-size: 16px;">
    ...
    <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
      <div class="row">
        ...
        <div class="d-grid gap-2 d-md-flex justify-content-md-center">
          ...
          <button type="button" class="btn btn-outline-primary mt-3 mb-3" data-bs-toggle="modal" data-bs-target="#archive"><i class="bi bi-cloud-upload-fill"></i> ARCHIVO</button>
          ...
          <button type="button" class="btn btn-outline-danger mt-3 mb-3" data-bs-toggle="modal" data-bs-target="#documento"><i class="bi bi-cloud-upload-fill"></i> DOCUMENTO</button>

          <div class="modal fade" id="archive" tabindex="-1" aria-labelledby="archive" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="archive">Subir Archivo</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                <form id="archive" action="actions/uploads/upload-archive.php?id=<?= $_SESSION['user']['id'] ?>" method="POST" enctype="multipart/form-data">
                <div class="d-flex flex-column align-items-center p-5 bg-light border-primary border-2 dropzone" style="border: dashed;">
                  <div class="dropzone">Arrastra y suelta el archivo aquí</div>
                    <div id="filePreview"></div>
                      <label class="btn btn-primary mt-3">
                        <span id="fileCounter">Seleccionar Archivo</span>
                        <input type="file" id="fileInput" name="archivo" style="display: none;" accept=".txt, .zip, .rar, .html, .css, .js, .php, .py, .c, .cpp, .java, .rb, .pl, .go, .d, .xml, .yml, .json, .md, .rules">
                      </label>
                      <script>
                        const dropzoneArchive = document.querySelector('.dropzone');
                        const filePreviewArchive = document.querySelector('#filePreview');
                        const fileCounterArchive = document.querySelector('#fileCounter');
                        const fileInputArchive = document.querySelector('#fileInput');

                        dropzoneArchive.addEventListener('drop', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                          fileInputArchive.files = e.dataTransfer.files;
                          fileInputArchive.dispatchEvent(new Event("change"));
                        });
                        let archive;
                        dropzoneArchive.addEventListener('dragover', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                        });
                        dropzoneArchive.addEventListener('drop', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                          archive = e.dataTransfer.files[0];
                          renderPreviewAndCounterArchive();
                        });
                        fileInputArchive.addEventListener('change', function(e) {
                          archive = e.target.files[0];
                          renderPreviewAndCounterArchive();
                        });
                        function renderPreviewAndCounterArchive() {
                          filePreviewArchive.innerHTML = '';
                          if (archive) {
                            const filePreviewElementArchive = document.createElement('div');
                            filePreviewElementArchive.innerHTML = `Nombre: ${archive.name} - Tamaño: ${(archive.size / 1024 / 1024).toFixed(5)} MB`;
                            filePreviewArchive.appendChild(filePreviewElementArchive);
                            fileCounterArchive.innerHTML = `Seleccionar otro archivo`;
                          } else {
                            fileCounterArchive.innerHTML = `Seleccionar Archivo`;
                          }
                        }
                      </script>
                    </div>
            ...
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                ...
                <div class="d-flex flex-column align-items-center p-5 bg-light border-success border-2 dropzoneMultimedia" style="border: dashed;">
                  <div class="dropzoneMultimedia">Arrastra y suelta multimedia aquí</div>
                    <div id="filePreviewMultimedia"></div>
                      <label class="btn btn-success mt-3">
                        ...
                        <input type="file" id="fileInputMultimedia" name="multimedia" style="display: none;" accept=".jpg, .jpeg, .png, .gif,.bmp, .tiff, .mp3, .wav, .aac, .flac, .m4a, .mp4, .avi, .mkv, .flv, .mov, .wmv, .gif, .midi, .aiff, .dvd, .bluray, .srt, .svg">
                      </label>
                      <script>
                        ...
                        function renderPreviewAndCounterMultimedia() {
                          ...
                          if (multimedia) {
                            ...
                            filePreviewElementMultimedia.innerHTML = `Nombre: ${multimedia.name} - Tamaño: ${(multimedia.size / 1024 / 1024).toFixed(5)} MB`;
          ...
          <div class="modal fade" id="documento" tabindex="-1" aria-labelledby="documento" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="documento">Subir Documento</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                <form id="documento" action="actions/uploads/upload-document.php?id=<?= $_SESSION['user']['id'] ?>" method="POST" enctype="multipart/form-data">
                <div class="d-flex flex-column align-items-center p-5 bg-light border-danger border-2 dropzoneDocument" style="border: dashed;">
                  <div class="dropzoneDocument">Arrastra y suelta un documento aquí</div>
                    <div id="filePreviewDocument"></div>
                      <label class="btn btn-danger mt-3">
                        <span id="fileCounterDocument">Seleccionar Documento</span>
                        <input type="file" id="fileInputDocument" name="documento" style="display: none;" accept="doc, docx, rtf, pdf, xls, xlsx, csv, ppt, pptx, pps, gdoc, gsheet, gslides, odt, ods, odp">
                      </label>
                      <script>

                        const dropzoneDocument = document.querySelector('.dropzoneDocument');
                        const filePreviewDocument = document.querySelector('#filePreviewDocument');
                        const fileCounterDocument = document.querySelector('#fileCounterDocument');
                        const fileInputDocument = document.querySelector('#fileInputDocument');

                        dropzoneDocument.addEventListener('drop', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                          fileInputDocument.files = e.dataTransfer.files;
                          fileInputDocument.dispatchEvent(new Event("change"));
                        });
                        let documento;
                        dropzoneDocument.addEventListener('dragover', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                        });
                        dropzoneDocument.addEventListener('drop', function(e) {
                          e.preventDefault();
                          e.stopPropagation();
                          documento = e.dataTransfer.files[0];
                          renderPreviewAndCounterDocument();
                        });
                        fileInputDocument.addEventListener('change', function(e) {
                          documento = e.target.files[0];
                          renderPreviewAndCounterDocument();
                        });
                        function renderPreviewAndCounterDocument() {
                          filePreviewDocument.innerHTML = '';
                          if (documento) {
                            const filePreviewElementDocument = document.createElement('div');
                            filePreviewElementDocument.innerHTML = `Nombre: ${documento.name} - Tamaño: ${(documento.size / 1024 / 1024).toFixed(5)} MB`;
                            filePreviewDocument.appendChild(filePreviewElementDocument);
                            fileCounterDocument.innerHTML = `Seleccionar otro documento`;
                          } else {
                            fileCounterDocument.innerHTML = `Seleccionar Documento`;
                          }
                        }
                      </script>
                    </div>
          ...
          <?php foreach ($user_texts as $user_texts_update) : ?>
            <?php $text_title = $user_texts_update['texts_title'];?>
            <?php $text_id = $user_texts_update['id'];?>
            <?php $text_text = $user_texts_update['texts'];?>

          <div class="modal fade" id="text-update" tabindex="-1" aria-labelledby="text-update" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered modal-lg">
              <div class="modal-content">
                <div class="modal-header">
                  <h1 class="modal-title fs-5" id="modalTextTitle">Editar Textos</h1>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  <strong id="CharacterTextUpdateCount">5000 caracteres restantes</strong>
                  <form id="form" action="actions/uploads/upload-text.php?id=<?= $_SESSION['user']['id'] ?>" method="POST" enctype="multipart/form-data">
                    <div class="mb-3">
                      <label for="text_title" class="form-label"></label>
                      <input type="text" class="form-control" id="textTitle" name="textTitle" placeholder="Titulo del Texto" value="<?=$text_id?>" required>
                    </div>
                    <div class="form-floating">
                      <textarea class="form-control" placeholder="Escribe aqui el texto que deseas guardar" id="textAreaUpdate" name="textAreaUpdate" style="height: 500px" maxlength="5000" required ><?=$text_text?></textarea>
                      <label for="textAreaUpdate">Escribe aqui el texto que deseas guardar</label>
                    </div>
                    <div class="modal-footer">
                    <button type="button" class="btn btn-outline-danger" data-bs-dismiss="modal">Cerrar</button>
                      <button type="submit" class="btn btn-primary">Guardar</button>
                  ...
                  <script>
                    ...
                    const CharacterTextUpdateCount = document.querySelector("#CharacterTextUpdateCount");

                    textareaupdate.addEventListener("input", function() {
                      const charactersLeftText = 5000 - textareaupdate.value.length;
                      CharacterTextUpdateCount.innerHTML = charactersLeftText + " caracteres restantes";

                      if (charactersLeftText <= 0) {
                        CharacterTextUpdateCount.classList.add("text-danger");
                      } else if (charactersLeftText <= 30) {
                        CharacterTextUpdateCount.classList.add("text-warning");
                        CharacterTextUpdateCount.classList.remove("text-danger");
                      } else {
                        CharacterTextUpdateCount.classList.remove("text-danger");
                        CharacterTextUpdateCount.classList.remove("text-warning");
                      }
                    });
                  </script>
                </div>
          ...
          <div class="col-sm-2 mt-2  text-center">
                <div class="card">
                  ...
                  <?php
                    ...
                    <button type="button" class="btn btn-info btn-sm mb-1"><i class="bi bi-info-circle-fill" type="button" data-bs-toggle="popover" data-bs-placement="right" data-bs-custom-class="custom-popover" data-bs-title="Informacion del Archivo" data-bs-content="Tamaño: <?= $user_record["size"]?> Fecha: <?= $user_record["upload_date"] ?>"></i></button>
                  </div>
                </div>
              </div>
        <?php endforeach ?>
          ...
          <div class="col-sm-2 mt-2  text-center">
                <div class="card">
                  <div class="card-body">
                    ...
                    <p><?= $text_title ?></p>
                    <a href="sections/text-edit.php?id=<?=$text_id?>"><button type="button" class="btn btn-primary btn-sm mb-1" data-id="<?=$text_id?>"><i class="bi bi-eye-fill"></i></button></a>
                    <a href="sections/text-edit.php?id=<?=$text_id?>"><button type="button" class="btn btn-warning btn-sm mb-1" data-id="<?=$text_id?>"><i class="bi bi-pencil-fill"></i></button></a>  
        ...
        <!-- <table class="table">
          ...
          <?php foreach ($all_user_records as $user_record) : ?>
              ...
              <tr>
                ...
                <td><?= $user_record["storage_directory"] ?></td>
                <td><?= $filename ?></td>
                <td><?= $user_record["size"] ?></td>
                <td>
                  <?php
                    $path_info = pathinfo($user_record["storage_directory"]);
                    $extension = strtolower($path_info["extension"]);
                ...
                <td><?= $user_record["upload_date"]?></td>
                <td><a href="actions/download/download.php?file=<?=$file_path?>">Descargar</a></td>
                <td><i class="bi bi-info-circle-fill" type="button" data-bs-toggle="popover" data-bs-placement="right" data-bs-custom-class="custom-popover" data-bs-title="Informacion del Archivo" data-bs-content="Tamaño: <?= $user_record["size"]?> Fecha: <?= $user_record["upload_date"] ?>"></i></td>
                <td><?=$user_record["table_name"]?></td>
                <td><a href="actions/delete/delete.php?file=<?=$file_path?>&id=<?=$user_record['id']?>&table=<?=$user_record["table_name"]?>">Eliminar</a></td>
              </tr>
            <?php endforeach ?>
